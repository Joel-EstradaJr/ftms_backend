/**
 * Admin Revenue List Page
 * Implements Task 2.1.1: Update Revenue List Page
 * 
 * Displays revenue records with:
 * - Updated table columns (revenueCode, transactionDate, source, paymentMethod, arStatus, etc.)
 * - Enhanced filters (source, external type, AR status, installments, date range)
 * - Status badges (AR status, JE status, installment indicator)
 * - Action buttons (View, Edit, Delete)
 * 
 * Backend: GET /api/admin/revenue (Task 1.1.1 completed)
 */
...
// Import new types from centralized revenue types
import type { Revenue, RevenueSource, PaymentMethod, JournalEntry } from "@/app/types/revenue";
...
const AdminRevenuePage = () => {
  ...
  const [externalTypeFilter, setExternalTypeFilter] = useState<string[]>([]);
  const [arStatusFilter, setArStatusFilter] = useState<string[]>([]);
  const [hasARFilter, setHasARFilter] = useState(false);
  const [hasInstallmentFilter, setHasInstallmentFilter] = useState(false);
  ...
  // Fetch revenue sources for filter dropdown
  const fetchSources = async () => {
    try {
      const response = await fetch('/api/admin/revenue-sources');
      if (!response.ok) throw new Error('Failed to fetch revenue sources');
      const sourcesData = await response.json();
      setSources(sourcesData.filter((s: RevenueSource) => s.isActive));
    } catch (error) {
      console.error('Error fetching revenue sources:', error);
      showError('Failed to load revenue sources', 'Error');
    }
  };
