...
export default function StaffMicroservicePage() {
  ...
  if (isLoading) {
    return (
      <div className="card">
        <h1 className="title">{microservice.name} (Staff)</h1>
        <Loading />
      </div>
    );
  }
  ...
  // Construct the iframe URL with staff context
  const iframeUrl = `${microservice.url}/${route}?role=staff`;
